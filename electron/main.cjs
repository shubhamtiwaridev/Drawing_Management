...
async function waitForAppReady(port, host = "127.0.0.1", timeoutMs = 30000) {
  const startedAt = Date.now();
  const healthUrl = `http://${host}:${port}/api/health`;
  const frontendUrl = `http://${host}:${port}/login`;

  while (Date.now() - startedAt <= timeoutMs) {
    try {
      const [healthRes, frontendRes] = await Promise.all([
        httpGet(healthUrl, 3000),
        httpGet(frontendUrl, 3000),
      ]);

      const contentType = String(
        frontendRes.headers["content-type"] || "",
      ).toLowerCase();

      if (
        healthRes.statusCode === 200 &&
        frontendRes.statusCode === 200 &&
  ...
  throw new Error(`Frontend did not become ready within ${timeoutMs}ms`);
}
...
async function createWindow() {
  ...
  await waitForAppReady(backendPort, "127.0.0.1", 30000);
  await mainWindow.loadURL(`http://127.0.0.1:${backendPort}/login`);
